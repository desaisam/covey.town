...
describe('CoveyTownController', () => {
  ...
  describe('addPlayer', () => {
    ...
    it('should use the coveyTownID and player ID properties when requesting a video token', async () => {
      const townName = `FriendlyNameTest-${nanoid()}`;
      const townController = new CoveyTownController(townName, false);
      const newPlayerSession = await townController.addPlayer(new Player(nanoid(), 'avatar1'));
  ...
  describe('town listeners and events', () => {
    ...
    it('should notify added listeners of player movement when updatePlayerLocation is called', async () => {
      const player = new Player('test player', 'avatar2');
    ...
    it('should notify added listeners of player disconnections when destroySession is called', async () => {
      const player = new Player('test player', 'avatar8');
    ...
    it('should not notify removed listeners of player movement when updatePlayerLocation is called', async () => {
      const player = new Player('test player', 'avatar5');
      await testingTown.addPlayer(player);
    ...
    it('should not notify removed listeners of player disconnections when destroySession is called', async () => {
      const player = new Player('test player', 'avatar6');
      const session = await testingTown.addPlayer(player);
    ...
    it('should not notify removed listeners that the town is destroyed when disconnectAllPlayers is called', async () => {
      const player = new Player('test player', 'avatar7');
      await testingTown.addPlayer(player);
  ...
  describe('townSubscriptionHandler', () => {
    ...
    beforeEach(async () => {
      ...
      player = new Player('test player', 'avatar6');
      ...
      describe('when a socket disconnect event is fired', () => {
        it('should remove the town listener for that socket, and stop sending events to it', async () => {
          ...
          if (disconnectHandler && disconnectHandler[1]) {
            disconnectHandler[1]();
            const newPlayer = new Player('should not be notified', 'avatar6');
