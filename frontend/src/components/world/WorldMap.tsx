...
class CoveyGameScene extends Phaser.Scene {
    ...
    if (this.id !== myPlayer.id && this.physics && player.location) {
      ...
      if (!sprite) {
        sprite = this.physics.add
          ...
          // @ts-ignore - JB todo
          // Players avatar
          .sprite(0, 0, 'atlas', `${this.avatar}_front`)
          .setSize(30, 40)
          .setOffset(0, 24);
      ...
      if (player.location.moving) {
        sprite.anims.play(`${this.avatar}_${player.location.rotation}_walk`, true);
      } else {
        sprite.anims.stop();
        sprite.setTexture('atlas', `${this.avatar}_${player.location.rotation}`);
      }
    }
  }
  ...
  create() {
    ...
    const sprite = this.physics.add
      .sprite(spawnPoint.x, spawnPoint.y, 'atlas', `monk_front`)
      .setSize(30, 40)
      .setOffset(0, 24);
...
export default function WorldMap(): JSX.Element {
  ...
  const {
    emitMovement, players,myPlayerAvatar,currentTownID
  } = useCoveyAppState();
  // console.log("Checking Players array");
    // console.log(myPlayerAvatar);
  ...
  useEffect(() => {
    ...
    if (video) {
      const newGameScene = new CoveyGameScene(video, emitMovement, myPlayerAvatar);
